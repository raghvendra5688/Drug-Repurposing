...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run a SMILES autoencoder on the output file (with the full path) generated from prepare_smiles_autoencoder.py')
    ...
    datafields = [("src", src), # we won't be needing the id, so we pass in None as the field
                  ("trg", trg)]
    data = TabularDataset(
               path=args.input1, # the file path
               format='csv',
    ...
    print("SMILES autoencoder training done")
