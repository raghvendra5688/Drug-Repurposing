...
def get_compound_viral_protein_interactions(input1,input2,output):

    #+
    #Get the compound - virus interaction network
    compound_viral_protein_info = pd.read_csv(input1,header='infer',sep="\t")
    ...
    full_viral_proteins = pd.read_csv(input2,compression="gzip")
    ...
    subset3_compound_viral_protein_info.to_csv(output,index=False)
    subset3_compound_viral_protein_info
    print("Finished producing compound-viral protein interactions from ChEMBL")
    # -

if __name__ == '__main__':
    
    parser = argparse.ArgumentParser(description='Obtain compound-viral protein interactions from ChEMBL')
    parser.add_argument('input1', help='input compound-viral protein interactions file from ChEMBL')
    parser.add_argument('input2', help='input viral protein sequence file from Uniprot')
    parser.add_argument('output', help='Filtered compound-viral protein interactions file for ChEMBL')
    args = parser.parse_args()

    get_compound_viral_protein_interactions(args.input1,args.input2,args.output)
